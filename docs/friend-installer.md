...
3. 按页面逐步创建自己的 Spotify 应用，添加 `http://127.0.0.1:18787/auth/spotify/callback`，只选择 Web API，粘贴自己的 Client ID 并授权。开发模式要求应用所有者拥有有效 Premium。这一步需要本人操作 Spotify 后台，助手不能代替接受条款。
   此时账号卡片下方的“立即同步一次”即可使用，不依赖 GitHub。确认后在本机创建或替换专用歌单；保持 App 运行至完成。预算或网络暂停会保留进度，等待结束后手动继续。
...
- 不建立本地音乐同步定时器。部署前允许显式手动同步；云端接管后手动入口只提交云端任务，本机不能直接刷新音乐令牌。重新授权先停用云端工作流、确认没有在运行的任务，再读取最新令牌并更新配置。已开始但未完成的云端配置也禁止本机同步。
