...
- On upgrade, only the legacy `request-budget` local deadline is retired. Real Spotify cooldowns, network backoff, request reservations and matching checkpoints remain intact. An expected safety pause exits cleanly with an explicit pending summary, not a misleading generic error; only a completed playlist update records a successful sync date. Hourly recovery resumes eligible pending work.
