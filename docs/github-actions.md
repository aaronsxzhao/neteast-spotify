...
- Spotify requests are serialized, at least 2 seconds apart, with at most 60 outgoing requests per sync attempt and per rolling hour. Authorization, search, and playlist writes all count. Reservations are encrypted and saved before sending, so restarting Actions or using force cannot reset the rolling budget. These are conservative application limits, not Spotify's published quota or a guarantee against 429. Spotify's limits vary; see [Spotify rate limits](https://developer.spotify.com/documentation/web-api/concepts/rate-limits).
...
- A 5xx or network failure stops rather than restarting the whole sync. A separate local backoff begins at 15 minutes and doubles on consecutive failures, capped at 4 hours, and never shortens a server-supplied wait. This local safety pause is distinct from Spotify's cooldown. Force cannot bypass either pause or the budget.
...
- Budget exhaustion pauses without publishing an incomplete playlist. Large recommendation sets can take multiple hourly runs. Playlist write retries reuse completed matches; replacement uses idempotent PUT for the normal daily playlist (up to 100 tracks).
