...
- 默认在北京时间约 **07:00** 运行，保留 08:00、09:00、10:00 补查；成功后跳过当天重复同步。GitHub 使用 UTC 配置，07:00 对应前一 UTC 日期的 23:00，同步日期仍按北京时间计算。
- 每小时第 05、20、35、50 分钟检查遗漏或失败任务。当天早上 07:00 后尚未成功时尝试补跑；已有待恢复任务也会在冷却结束后续跑。GitHub 可能延迟或漏掉计划任务，这不是准点保证。
- GitHub 定时可能延迟或漏发，不保证准点。页面中的“立即同步”可提交补跑，但不会强行绕过冷却。
- 关闭电脑不影响已部署的云端任务；首次验证仍须确认 GitHub 运行环境能够访问自己的网易云账号。

### 匹配策略

默认歌单标题固定为 `NetEase Daily`（旧默认名 `NetEase Daily Recommendations` 会自动迁移，其他自定义标题保留）。日期、匹配数量写在描述中，不追加到标题。每日同步只更新同一歌单的歌曲及名称／描述／可见性，不上传或覆盖你手动设置的封面；只有显式点击本地“应用封面”才会更换封面。
...
5. 已核验的歌手别名可立即用于确认已有候选；额外的人工别名查询、人工歌名翻译仍作为最后补充。不确定、无版权或地区不可用的歌曲可能仍未匹配。

版本解析支持 `-band ver-`、紧贴破折号的版本标签和 `- 2015 Remaster` 等写法，保留普通副标题。跨语言名称优先使用元数据；无法可靠推导时使用核验过的映射，不仅凭时长猜测翻译。表演组合署名（例如特定曲目的 band set）只对核验过的来源曲目生效，不作为全局歌手别名。

版本括号也支持 `＜Billboard Live Ver.＞`、`〈…〉` 和 `《…》`；只有包含明确版本标签才去除。已核验的艺人对应关系可双向、跨歌曲复用，但不会把 `DJ` 前缀或不同发行者署名自动当成同一人。单艺人的跨文字斜杠署名（例如 `露崎 春女/Lyrico`），以及完整的 `角色(CV:声优)` 演唱名单，仅在标题、专辑高度一致、时长差不超过 2.5 秒时用于该录音的身份佐证；不生成全局别名，也不把一位声优视为整个组合。映射依据与限制见 [身份核验记录](docs/catalog-identities.md)。
...
- 已找到无歧义的严格匹配，标题相似度至少 0.98、同一主要歌手且时长差不超过 2.5 秒时，立即停止搜索；合辑名称不同不必继续查。仅靠推测的跨语言歌手关系不能触发这种提前结束。
- 每首最多 18 次目录查询，已知 ID 验证、普通搜索、专辑遍历和二次检查共享上限。各策略先获得初始额度，剩余额度可用于未执行的高价值查询；只有真正执行过的查询才跨阶段去重。双语独立标题优先于标点变体；连续两次没有有效匹配证据就切换策略，无关新歌曲、封面和热度变化不会延长该阶段。缓存查询也保守计入，但令牌刷新、歌单写入、网络重试不在这 18 次内，仍受全局请求预算约束。
- 新 ID 的同名弱候选如果没有提高证据等级，不再重置阶段停止计数。各专门策略及允许的版本复核结束后，若已有标题与时长高度吻合、但艺人身份仍未确认的候选，则跳过最后的关键词排列补查，标记 `identityReviewRequired`，而不是继续用满预算或宣称歌曲不存在。纯粹尚未召回的歌曲仍保留剩余额度补查；此调整不改变 Spotify 冷却、请求间隔或每日调度。
- 游戏主题曲副标题可生成短标题检索，但不会因此放宽歌名评分。已核验的艺人名优先进入别名补查；复合艺人名称拆词只帮助搜索，不能把 `Cold Diamond & Mink` 误认成歌手 mink。
- 已知 Spotify 曲目 ID 只作为检索提示，每次重新同步仍需验证账号可播放性和歌曲身份。地区不可用、可播放性未知、身份不明及预算受限分别保留诊断；具体查询轨迹仅加密保存，不进入公开 Actions 日志。
- 成功写入后保存跨天曲目 ID 提示（最多 400 条、有效期 30 天），按源歌曲完整元数据和应用／歌单隔离。之后优先用一次 ID 请求重新核验，再按当前规则评分；失效、不可播放或不匹配则回到普通检索。缓存不保存“未匹配就是没有”的结论，不直接复用过期匹配，也不绕过限流。
- `(with 合作歌手)` 只有在合作方确实出现在结构化艺人列表时才可从标题评分中移除；普通副标题仍保留。合作名单中的共享嘉宾不能冒充主要歌手；同主要歌手、接近时长的多个发行版本优先参考源专辑，但不能压过明显更准确的时长证据。
- `Remaster`、`【已售】`、逐字装饰空格会生成更干净的优先检索词，原始标题和版本信息仍用于身份核验。不能从元数据推导的翻译继续使用限定源歌曲的人工核验提示，而不是给所有同名歌曲套用翻译。
- 标题限定和不限定歌手的搜索后，确认同歌同主要歌手的其他版本也可提前结束；接受前先重评已有候选，优先更接近原版的录音。预算截断的未匹配会标记为待复核，不能理解为 Spotify 没有这首歌。
- 元数据检索失败后，先用已核验的名称对应关系重评已有候选；同歌同主要歌手的可接受版本也可直接复用，避免再发别名或版本查询。此优化不新增 Spotify 请求，不绕过冷却或放宽其他歌手的匹配要求。
