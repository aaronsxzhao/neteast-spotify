...
> 请使用 **v1.1.0-beta.2 或更新版本**。beta.1 的 Mac 包存在依赖打包缺陷，会导致网易云扫码报错；新版已修复，并在打包后实际检查网易云模块加载与二维码生成。
...
3. 浏览器会自动打开安装界面。按顺序连接网易云、Spotify，再确认一次 GitHub 授权。
   两个音乐账号连接好后，已可点击账号卡片下方的 **“立即同步一次”**，不需要 GitHub。首次运行会确认创建或替换专用歌单；本机同步期间请保持 App 运行。
...
只想手动同步，可以暂不配置 GitHub；要每天自动运行，再完成云端授权。云端接管后，手动入口会改为“发起云端同步”，不会再由本机使用旧令牌同步。同步中禁止同时部署或重新连接；本机请求预算暂停后需到时点击继续，不会自行定时重试。部署会保留现有歌单、当日成功记录与未完成进度。
...
- 默认在北京时间约 **08:00** 运行，09:00、10:00 补查；成功后跳过当天重复同步。GitHub 使用 UTC 配置，实际启动可能延迟。
- 每小时第 05、20、35、50 分钟检查遗漏或失败任务。当天早上 08:00 后尚未成功时尝试补跑；已有待恢复任务也会在冷却结束后续跑。GitHub 可能延迟或漏掉计划任务，这不是准点保证。
...
- 程序将请求均匀限速：至少间隔 6 秒、滚动 30 秒最多 5 次；取消原先 60 次／小时的硬暂停。单次 300 次是防失控上限，触达后保存进度并暂停 15 分钟。只读搜索遇到网络错误或 `5xx` 时，在原任务内最多重试两次（基础等待 2 秒、8 秒，仍受限速与服务端等待时间约束）；持续故障才保存长退避。授权与歌单写入不会自动重放。
- 已明确匹配同标题、同主歌手、同专辑、时长差不超过 1 秒的结果时，提前结束别名查询；跨语言、模糊或有歧义的结果继续完整检索和二次检查。
