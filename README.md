...
- 每首最多 18 次目录查询，已知 ID 验证、普通搜索、专辑遍历和二次检查共享上限。各策略先获得初始额度，剩余额度可用于未执行的高价值查询；只有真正执行过的查询才跨阶段去重。双语独立标题优先于标点变体；连续两次没有新候选就切换策略。缓存查询也保守计入，但令牌刷新、歌单写入、网络重试不在这 18 次内，仍受全局请求预算约束。
